...
account_router = Blueprint('accounts', __name__)
...
@account_router.route('/<account_id>', methods=['PUT'])
@auth_service.token_required
def update_account(current_user, account_id):
    data = request.json
    
    updated_account, error = account_service.update_account(account_id, current_user['id'], data)
...
def delete_account(current_user, account_id):
    success, error = account_service.delete_account(account_id, current_user['id'])
